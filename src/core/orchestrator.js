...
const SCHEMA_MIGRATIONS_REGISTRY = [
    {
...
export async function runMuraqibUpgradeOrchestrator({ packageName, currentValue, newVersion, rangeStrategy, remotePresetUrl, }) {
    ...
    let activePresets = MURAQIB_LOCAL_PRESETS;
    ...
    const matchedPreset = activePresets.find(preset => preset.packages.includes(packageName));
    if (matchedPreset) {
    ...
    if (updatedVersionValue && updatedVersionValue !== currentValue) {
        ...
        const migrationRule = SCHEMA_MIGRATIONS_REGISTRY.find((rule) => rule.packageName === packageName && targetMajor === rule.breakingMajor);
        if (migrationRule) {
