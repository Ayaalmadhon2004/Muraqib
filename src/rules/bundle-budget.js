 ...
 */
export const checkLazyLoadingNecessity = (filePath) => {
    ...
    heavyComponents.forEach(component => {
        ...
        if (isComponentUsed && !isAlreadyLazy) {
            suggestions.push(`💡 [Muraqib Suggestion]: Heavy component '${component}' detected in workspace. Consider using lazy/dynamic loading to preserve initial bundle budget:\n` +
 ...
 */
export const checkHeavyImports = (filePath) => {
    if (!fs.existsSync(filePath))
        return [];
    const fileContent = fs.readFileSync(filePath, 'utf-8');
    const suggestions = [];
    if (fileContent.includes("import _ from 'lodash'") || fileContent.includes("import lodash from 'lodash'")) {
        suggestions.push(`⚠️ [Muraqib Optimization]: You are importing the ENTIRE 'lodash' library! This breaks your 14KB first-byte budget.\n` +
    ...
    if (fileContent.includes("import * as Icons from '@mui/icons-material'")) {
        suggestions.push(`⚠️ [Muraqib Optimization]: Importing global MUI icons namespace will heavily bloat your compile bundle size.\n` +
 ...
 */
export const checkMinificationSettings = (projectRoot) => {
    const nextConfigPath = path.join(projectRoot, 'next.config.js');
    const suggestions = [];
    if (fs.existsSync(nextConfigPath)) {
        const configContent = fs.readFileSync(nextConfigPath, 'utf-8');
        if (configContent.includes('swcMinify: false')) {
            suggestions.push(`🚨 [Muraqib Critical]: Minification optimization is explicitly disabled ('swcMinify: false')!\n` +
...
export const runComprehensiveBundleAudit = () => {
    ...
    function findActiveWorkspaceFile(dir) {
