...
// =========================================================================
export const auditPerformance = (resourceCount, protocol, cookiesSize) => {
    const findings = [];
...
export const analyzeRenderBlocking = (htmlContent) => {
    const headMatch = htmlContent.match(/<head>[\s\S]*?<\/head>/i);
    if (!headMatch)
        return { status: 'ok', message: 'لم يتم العثور على <head>' };
...
function runOptimizerAudit(targetPath) {
    let htmlContent = "";
    try {
    ...
    const protocol = /http\/2|h2|HTTP\/2/i.test(htmlContent) ? "HTTP/2" : "HTTP/1.1";
...
function runRenderBlockingAudit(targetPath) {
    let htmlContent = "";
    try {
        const htmlPath = path.join(targetPath, "index.html");
        if (fs.existsSync(htmlPath)) {
    ...
    if (result.blockingStyles > 0) {
        reports.push(`ملاحظة: لديك ${result.blockingStyles} ملف CSS blocking في الـ head. فكّري باستخدام media queries أو preload.`);
    }
    return {
...
`);
    ...
    if (allOk) {
        ...
        const warningCount = result.memory.errors.length +
...
// =========================================================================
async function runUpgradePackages(targetPath) {
...
// =========================================================================
function extractEnvErrors(error) {
    if (error?.isMuraqibCustom && Array.isArray(error.errors)) {
        return error.errors.map((e) => `${e.path || e.field || "unknown"}: ${e.message || "invalid"}`);
    }
